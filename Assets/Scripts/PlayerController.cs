    ...
    void Update() {
        if (!ActivateDash) {
            ...
            if (upperWall != null && lowerWall != null) {
                if (lowerWall.transform.position.y < transform.position.y) {
                    lowerWallRenderer.sortingOrder = spriteRenderer.sortingOrder + 2;
                Debug.Log("Low1");
                }
                else {
                    lowerWallRenderer.sortingOrder = spriteRenderer.sortingOrder - 1;
                Debug.Log("Low2");
                }

                if (upperWall.transform.position.y < transform.position.y) {
                    upperWallRenderer.sortingOrder = spriteRenderer.sortingOrder + 1;
                    Debug.Log("Up1");
                }
                else {
                    upperWallRenderer.sortingOrder = spriteRenderer.sortingOrder - 1;
